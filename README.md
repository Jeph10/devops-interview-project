...
把任务 1–4 的 `task-api` 想象成一个更大线上系统的一部分。系统里还有一张高频写入的用户事件表 `user_event`：写入 **DynamoDB**，并把数据备份/同步到一个 **S3** 数仓桶，下游用 **EKS** 上的服务消费这些事件，对外流量走 **ELB**。最近 APP 做了一次较大改版，客户端的请求逻辑随之改变，之后出现如下情况：

- 本月 **AWS 账单环比上涨约 30%**（示意：约 $40k → $52k）；
- 同期**业务量（DAU / 核心请求量）只增长约 5%**；
- 增量不只落在一个服务上，而是**分散在 DynamoDB、S3、EKS 计算、ELB 出口**等多个服务；
