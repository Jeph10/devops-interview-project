...
> 这是一道**场景分析题**，不需要写代码，也不要求购买云资源。把分析写入 `deploy/COST.md`（建议不超过 800 字）。**能拿到数据就给出具体数字和查询；拿不到就写清关键假设、你会用什么数据验证，以及假设不成立时结论会怎么变。** 我们既看结论，也看推理过程。

把任务 1–4 的 `task-api` 想象成一个更大线上系统的一部分。系统里还有一张高频写入的用户事件表 `user_event`：写入 **DynamoDB**，并把数据备份/同步到一个 **S3** 数仓桶（这个桶同时被下游数据作业 EMR / 查询读取），下游还用 **EKS** 上的服务消费这些事件，对外流量走 **ELB**。最近 APP 做了一次较大改版，客户端的请求逻辑随之改变，之后出现如下情况：

- 本月 **AWS 账单环比上涨约 30%**（示意：约 $40k → $52k，unblended 口径，上月 31 天 / 本月 30 天）；
- 同期**业务量（DAU / 核心请求量）只增长约 5%**；
- 增量**分散在 DynamoDB、S3、EKS 计算、ELB 出口**等多个服务——初步看，一部分随写入量同步上升，也有一部分与用量并不成比例；
- 本季度初刚买过一批 RI / Savings Plans；
- 你能拿到：Cost Explorer、可导出的 CUR 成本用量明细、约 60% 资源打了 `team` / `env` 标签、CloudWatch 指标，以及你在任务 3 中建立的服务指标；
- 财务和你的 leader 想要两个答案：**这次为什么涨**，以及**下个月能不能降回来、怎么降**。

**在 `deploy/COST.md` 中给出：**

- **判断基线**：你先看哪些数据来判断“这次上涨是否正常”？如何区分“用量增长”与“效率下降 / 浪费”？用什么口径衡量（例如单位成本 cost per 事件 / 请求 / DAU）？
- **定位思路**：给一条“观察 → 假设 → 用什么数据验证”的排查链，把这 30% 的增量拆解到具体来源，至少覆盖：先按什么维度拆（服务 / usage type / 账号 / region / tag），tag 只覆盖 60% 时怎么补归因；增量分散在多个服务时，你怎么判断它们**是否同源**；每一步想排除的干扰（账期天数、RI/SP 摊销、一次性费用等）。
- **治理与取舍**：任选一条约束，给出可落地的第一步、备选方案，以及你为此放弃了什么——(1) 想用承诺折扣（RI / Savings Plan）或预留容量降成本，但这张事件表**下季度计划迁移到别的存储**；(2) 增量根源是改版后的请求 / 写入逻辑，**从源头减量需要 APP 团队配合、但对方本季度排期很紧**；(3) 上个月你判断“下月回落”，本月却**创了新高**（点优化被源头增长盖过），你如何对 leader **如实管理预期**。
- **经验佐证（可选但鼓励）**：如果你做过真实的成本优化，用一到两句说明 before/after 的量化结果，以及你如何确认这个改善确实来自你的改动。

**行为约束：**

- 结论落到“用哪份数据、怎么验证”；避免只写“关掉闲置资源 / 买 RI / 加生命周期策略”这类通用清单——同样的动作，要说清你**为什么判断它适用于本场景**；
- 区分“一次性点优化”和“根因治理 / 长效机制”，并说明你会怎么防止成本回退。
